...
import utime
import ubinascii
#================================ PARTE DE LA SECCION MQTT===================
# ─────────────────────────────────────────────
#  IDENTIFICACION / MQTT / WiFi
# ─────────────────────────────────────────────
...
def sub_cb(topic, msg):
    ...
    if topic == TOPIC_SUB_COFRE:
        ...
        if msg == b"bloqueado":
            print("Cofre bloqueado. Iniciando estudio en LCD...")

            mqtt_publicar(TOPIC_PUB_ARCADE, b"estudiando")
            mqtt_publicar(TOPIC_PUB_SESION, b"iniciada")

            # Inicia el temporizador cuando el cofre confirma bloqueo
            if estado in ("menu", "estudiar_cfg"):
                ir_a("estudiar_run")
            ...
            # Si se abre mientras se está estudiando, se considera interrupción
...
def mqtt_conectar():
    ...
    print("Conectando arcade a WiFi...", end="")
    ...
    if not sta.isconnected():
        ...
        while not sta.isconnected() and intentos < 40:
            utime.sleep_ms(500)
            print(".", end="")
            intentos += 1
    ...
    try:
        print("Conectando arcade a MQTT...")

        client_id = "arcade-" + obtener_chip_id()
        ...
        print("MQTT Arcade conectado!")
        print("Suscrito a:", TOPIC_SUB_COFRE)
        ...
        print("Error MQTT Arcade:", e)
        return False


def mqtt_publicar(topic, msg):
    if not MQTT_HABILITADO or mqtt_client is None:
        return
...
#================================ PARTE DE LA SECCION MQTT==============================================
    ...
    if MQTT_HABILITADO and wifi_ok and mqtt_client is not None:
        try:
            mqtt_client.check_msg() #lo q escucha el cofre
        except:
            pass
#================================ PARTE DE LA SECCION MQTT==============================================
